...
const catColor = (c: string) => CAT_COLOR[c] ?? "#4A4A4A";
        ...
        <div key={step.id} className="flex flex-col">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-bold mb-0.5" style={{ color: "#E0E0E0", ...mono }}>
                {step.name}
              </div>
              <div
            ...
            <span
              ...
              style={{
                color: step.on_fail === "abort" ? "#FF3333" : "#4A4A4A",
          ...
          {i < steps.length - 1 && (
            <div className="flex justify-center" style={{ margin: 0 }}>
              <ChevronRight size={10} style={{ color: "#1F1F1F", transform: "rotate(90deg)" }} />
            </div>
          )}
        </div>
      ))}
...
function ExecStatusIcon({ status }: { status: string }) {
  if (status === "completed") return <Check size={11} style={{ color: "#00FF41" }} />;
  if (status === "failed")    return <X size={11} style={{ color: "#FF3333" }} />;
...
function CreateModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const inputSt: React.CSSProperties = {
    background: "#000", border: "1px solid #1F1F1F", color: "#E0E0E0",
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: "#080808", border: "1px solid #1F1F1F", maxHeight: "90vh", overflowY: "auto" }}
      >
        <div className="flex items-center justify-between mb-4">
          <span className="text-sm font-bold tracking-widest" style={{ color: "#00FF41", ...mono }}>
            // CREATE_PLAYBOOK
          </span>
          <button onClick={onClose} style={{ color: "#4A4A4A", background: "none", border: "none", cursor: "pointer" }}>
            <X size={14} />
          </button>
        </div>
        ...
        {err && (
          <div className="flex items-center gap-2 px-3 py-2 mb-3 text-xs"
            style={{ background: "rgba(255,51,51,0.05)", border: "1px solid rgba(255,51,51,0.2)", color: "#FF3333", ...mono }}>
        ...
        <div className="text-xs tracking-widest mb-2" style={{ color: "#2A2A2A", ...mono }}>// STEPS</div>

        <div className="flex flex-col gap-2 mb-3">
          {steps.map((step, i) => (
            <div key={i} className="flex gap-2 items-start">
              <div className="flex items-center justify-center text-xs flex-shrink-0"
                style={{ width: 22, height: 36, border: "1px solid #1F1F1F", color: "#4A4A4A", ...mono, fontSize: "10px" }}>
                {i + 1}
              </div>
              ...
              <button onClick={() => removeStep(i)} disabled={steps.length === 1}
                style={{ color: steps.length === 1 ? "#1F1F1F" : "#4A4A4A", background: "none", border: "none", cursor: steps.length === 1 ? "default" : "pointer", paddingTop: "8px" }}>
                <X size={12} />
              </button>
            </div>
          ))}
        ...
        <button onClick={addStep} className="w-full text-xs py-2 mb-4"
          style={{ background: "transparent", border: "1px dashed #1F1F1F", color: "#4A4A4A", cursor: "pointer", ...mono }}>
          + ADD STEP
        </button>
...
export default function Playbooks() {
  ...
  return (
    <div className="flex h-full" style={{ background: "#000", minHeight: "100vh" }}>

      {/* ── Left: Playbook list ── */}
      <div className="w-60 flex-shrink-0 flex flex-col" style={{ borderRight: "1px solid #1F1F1F", background: "#040404" }}>
        <div className="px-4 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid #1F1F1F" }}>
          <div>
            <div className="text-xs font-bold tracking-widest" style={{ color: "#00FF41", ...mono }}>
              // PLAYBOOKS
            </div>
            <div className="text-xs mt-0.5" style={{ color: "#2A2A2A", ...mono }}>
              {playbooks.length} loaded
            </div>
          </div>
          <div className="flex items-center gap-1">
            <motion.button whileTap={{ scale: 0.9 }} onClick={loadAll}
              style={{ color: "#4A4A4A", background: "none", border: "none", cursor: "pointer", padding: "4px" }}
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {loading && (
            <div className="flex justify-center p-6">
              <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
                style={{ width: 14, height: 14, border: "1px solid #00FF41", borderTopColor: "transparent", borderRadius: "50%" }} />
            </div>
          )}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className="text-xs font-bold truncate" style={{ color: active ? cc : "#9A9A9A", ...mono }}>
                    {pb.name}
                  </span>
                  {pb.built_in
                    ? <span className="text-xs flex-shrink-0" style={{ color: "#2A2A2A", ...mono, fontSize: "9px" }}>BUILT_IN</span>
                    : (
                      <button onClick={e => { e.stopPropagation(); deletePlaybook(pb); }}
                        style={{ color: "#2A2A2A", background: "none", border: "none", cursor: "pointer", padding: 0 }}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs" style={{ color: "#2A2A2A", ...mono, fontSize: "9px" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {!selected ? (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-xs" style={{ color: "#2A2A2A", ...mono }}>Select a playbook</p>
          </div>
        ) : (
          <>
            {/* Header */}
            <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div>
                <h1 className="text-base font-bold tracking-widest" style={{ color: "#00FF41", ...mono, textShadow: "0 0 8px rgba(0,255,65,0.4)" }}>
                  // {selected.name.toUpperCase().replace(/-/g, "_")}
                </h1>
                <p className="text-xs mt-1" style={{ color: "#4A4A4A", ...mono }}>{selected.description}</p>
                <div className="flex items-center gap-2 mt-2">
                  {selected.tags.map(t => (
                    <span key={t} className="text-xs px-1.5"
                      style={{ color: "#4A4A4A", border: "1px solid #1A1A1A", ...mono, fontSize: "9px" }}>
                      #{t}
                    </span>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-2 flex-shrink-0 ml-6">
                <select
                  ...
                  style={{
                    background: "#000", border: "1px solid #1F1F1F", color: agentId ? "#E0E0E0" : "#4A4A4A",
                ...
                <motion.button
                  ...
                  style={{
                    ...
                    border: `1px solid ${agentId ? "#00FF41" : "#1F1F1F"}`,
                    color: agentId ? "#00FF41" : "#2A2A2A",
                ...
                >
                  {running
                    ? <motion.div animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
                        style={{ width: 11, height: 11, border: "1px solid #00FF41", borderTopColor: "transparent", borderRadius: "50%" }} />
                    : <Play size={11} />}
                  RUN
                </motion.button>
              </div>
            ...
            {/* Tab bar */}
            <div className="flex flex-shrink-0" style={{ borderBottom: "1px solid #1F1F1F" }}>
                  ...
                  <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                    ...
                    {execLoading && (
                      <div className="flex items-center gap-2 px-3 py-2 text-xs"
                        style={{ background: "rgba(0,255,65,0.03)", border: "1px solid rgba(0,255,65,0.15)", color: "#00FF41", ...mono }}>
                    ...
                    {executions.filter(e => e.playbook_id === selected.id).length === 0 ? (
                      <div className="flex items-center justify-center py-16 text-xs" style={{ color: "#2A2A2A", ...mono }}>
                        [ NO EXECUTIONS YET ]
                      </div>
                    ) : executions
...
function ExecCard({ exe }: { exe: Execution }) {
  ...
  const statusColor = exe.task_status === "completed" ? "#00FF41"
    : exe.task_status === "failed" ? "#FF3333"
    : "#FFB800";
        ...
        <AnimatePresence>
          {open && exe.task_output && (
            <motion.pre
              ...
              style={{
                ...mono, fontSize: "10px", color: "#9A9A9A",
          ...
          {open && !exe.task_output && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              className="px-4 py-3 text-xs" style={{ color: "#2A2A2A", ...mono, borderTop: "1px solid #1A1A1A" }}>
